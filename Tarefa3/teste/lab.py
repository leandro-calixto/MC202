...
class labirinto(object):
	...
	def le_mapa(self, texto):
		...
		if not self.entrada:
			raise ValueError("O mapa nao possui uma entrada")
		if not self.saida:
			raise ValueError("O mapa nao possui uma saida")
	#le_mapa()

	def le_mapa_arquivo(self, arquivo):
		"""le um mapa de um arquivo"""
		f=open(arquivo)
		self.le_mapa(f.read())
		f.close()
	#le_mapa_arquivo()
	...
	def encontra_saida( linha, coluna ):
	   """Função recursiva para encontrar a saída."""
		...
		return True
		   ...
		   No caso iremos pela direita, depois para

		   cima, para esquerda e então para baixo.

		   Lembrando que isto é uma recursão e que

		   marcamos nosso caminho,

		   temos magicamente a lembrança dos caminhos
		...
		if not achou and \
			posicao_valida( linha, coluna + 1 )
			and \
			self.mapa linha ][ coluna + 1 ] in ( corredor, saida ):
		   """Ainda não encontrou e a posição à direita é corredor ou é a

		   saída. Prossiga pela direita."""

			achou = encontra_saida ( linha, coluna + 1)

		if not achou and \
			posicao_valida( linha-1, coluna )

			and\
			self.mapa linha - 1 ][ coluna ] in (corredor, saida ):
		   """Ainda não encontrou e a posição acima é corredor ou é a saída. Prossiga para cima."""
		...
		if not achou and \
			...
			and \
			self.mapa[ linha ][ coluna - 1] in (corredor, saida):
		   """Ainda não encontrou e a posição à esquerda é corredor ou é a saída. Prossiga para esquerda. """
		...
		if not achou and \
			...
			and \
			self.mapa[ linha + 1 ][ coluna ] in ( corredor, saida):
		   """Ainda não encontrou e a posição abaixo é corredor ou é a saída. Prossiga para baixo."""
		achou encontra_saida( linha + 1, coluna )

		#caso o caminho adotado foi correto, marque como solucao, se nao, marque como errado
